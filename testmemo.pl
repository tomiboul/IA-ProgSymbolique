...
/* Cas de base 3 : valeur présente dans le dico */
/*lev_list([H1 | T1], [H2 | T2], Dist, Matrice_Levenshtein, _) :-
    ...
    %%Value = Matrice_Levenshtein.get(Tuple_String, -1), 
    %get_dict(Tuple_String, Matrice_Levenshtein, Value),
    Value = Matrice_Levenshtein.get(Tuple_String),
    %write(" value of dic  : " +  Value),
    %write("aaaaaaaaaaaa"),
    Value \= -1,
    %write("ssssssssssssssssssssssssssssssss"),
    Dist = Value.
*/
lev_list([H1 | T1], [H2 | T2], Dist, Matrice_Levenshtein, Matrice_Levenshtein) :-
 ...
 lev_list([H1 | T1], [H2 | T2], Dist, Matrice_Levenshtein, New_Matrice_Levenshtein) :-
    ...
    %write(" \n Tuple string : " + Tuple_String).

    %%Matrice_Levenshtein.put((LengthA, LengthB), Dist)
